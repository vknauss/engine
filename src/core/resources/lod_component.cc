...
void LODComponent::addLOD(float maxScreenHeight, const Model* pModel) {
    ...
    if (m_lODs.size() == 1) {
        ...
        float dist = glm::distance(b.position, m_boundingSphere.position);
        m_boundingSphere = {
            glm::mix(b.position, m_boundingSphere.position, 0.5f * (1.0f + (b.radius - m_boundingSphere.radius) / dist)),
            0.5f * (b.radius + m_boundingSphere.radius + dist) };
    }
}
...
uint32_t LODComponent::getLOD(glm::mat4 frustumMatrix, glm::mat4 worldTransform) const {
    ...
    for (; level < m_lODs.size()-1; ++level) {
        float height = computeScreenHeight(m_boundingSphere.radius, frustumMatrix, worldTransform);
